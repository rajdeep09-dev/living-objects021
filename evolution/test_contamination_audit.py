...
def test_sorting_audit_detects_the_direct_sort_primitive_and_records_random_baseline() -> None:
    ...
    assert record["status"] == "RETRACTED_DIRECT_PRIMITIVE"
    assert {match["candidate"] for match in record["direct_solution_matches"]} == {"primitive:sort1"}
...
def test_reverse_and_game_contract_are_not_mislabelled_as_clean() -> None:
    ...
    assert game["status"] == "INVALID_EVALUATOR_CONTRACT"
...
def test_full_audit_lists_only_real_evaluators_and_marks_unimplemented_domains() -> None:
    ...
    assert report["summary"]["retracted_direct_primitive"] >= 2
