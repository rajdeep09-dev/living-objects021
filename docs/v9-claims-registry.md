...
| C-10 | v9–v10 | Bounded SDK, CLI, API contracts, five-stage curriculum, and signed discovery exchange are available; SDK version `0.3.0` repairs the tested public `fitness` and `source_code` result accessors | **IMPLEMENTED_TESTED** | Local/repository contracts only; evolved source remains an unexecuted audit export. This is **not a deployed federation**, autonomous worker, remote transport, or long-run discovery result | `living_objects/sdk.py`, `living_objects/test_sdk.py`, `living_objects/cli.py`, `production/api/v9/`, `docs/v10-foundation-audit.md` | A behavior claim requires a measured experiment separate from implementation coverage |
